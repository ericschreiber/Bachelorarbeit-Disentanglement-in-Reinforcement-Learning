...
# If you want to use a buffered VAE there are some comments which should help to get the code running with a buffered input
...
Breakout = str(sys.argv[8])

assert(bigAgent=='normal' or bigAgent=='big' or bigAgent=='superbig' or bigAgent=='superduperbig')

namefolder = namefolder +"-" + Breakout + "_" + bigAgent + "_" + str(features) + "_" + str(eps_decay) + "_" + str(random_seed)
...
class DQNsuperduperBig(nn.Module):
    def __init__(self, input_shape, n_actions):
        super(DQNsuperBig, self).__init__()

        self.fc = nn.Sequential(
            nn.Linear(features*2*4, 8200), 
            nn.InstanceNorm1d(3136),
            nn.ReLU(),
            #nn.Dropout(p=0.1),
...
class EncoderLikeDQNFULLBN(nn.Module):
    def __init__(self, output_dim: int, num_channels: int, latent_dim: int):
        super(EncoderLikeDQNFULLBN, self).__init__()
        self.output_dim = output_dim
        self.num_channels = num_channels
...
class DecoderLikeDQNFULLBN(nn.Module):
    def __init__(self, input_dim: int, num_channels: int, latent_dim: int):
        super(DecoderLikeDQNFULLBN, self).__init__()
        self.input_dim = input_dim
        self.num_channels = num_channels
...
class EncoderAELikeDQN(nn.Module):
    def __init__(self, output_dim: int, num_channels: int, latent_dim: int):
        super(EncoderAELikeDQN, self).__init__()
        self.output_dim = output_dim
        self.num_channels = num_channels
...
class VAE(nn.Module):
    def __init__(self, z_dim, num_channels, device, latent_dim):
        ...
        elif likeDQN == "likeDQNFULLBN":
            self.encoder = EncoderLikeDQNFULLBN(z_dim, num_channels, latent_dim) # use "wrong" encoder
            self.decoder = DecoderLikeDQNFULLBN(z_dim, num_channels, latent_dim)
        elif likeDQN == "AE":
            self.encoder = EncoderAELikeDQN(z_dim, num_channels, latent_dim) # use "wrong" encoder
            self.decoder = DecoderLikeDQNFULLBN(z_dim, num_channels, latent_dim)
        else:
...
if likeDQN == "AE":
    latentDim = features
    BVAE = AE(latentDim, 1, device, latentDim)
...
MEAN_REWARD_BOUND = -19   #Change to 19.0 !!!not used!!! #either use mean reward bound or max frame idx
...
writer = SummaryWriter(log_dir=f"/itet-stor/ericschr/net_scratch/BA/DQN_runs/With_Pretrained_AE/summary/{namefolder}", comment=f"-Buff{x.day}_{x.month}_EPSdec{eps_decay}" + DEFAULT_ENV_NAME) 
...
while True:
        ...
        if reward is not None:
            ...
            if frame_idx % 500 == 0:
                writer.flush #should give us constant updates for logging
            ...
            if best_mean_reward is None or best_mean_reward < mean_reward:
                ...
                if best_mean_reward is not None:
                    ...
                    torch.save(net.state_dict(), f"/itet-stor/ericschr/net_scratch/BA/DQN_runs/With_Pretrained_AE/summary/{namefolder}/" + DEFAULT_ENV_NAME + f"Buff-bestreward-{x.day}_{x.month}.dat")
        ...
        if linear == True:
            ...
            if likeDQN == "AE":
                states_preprocessed = states_preprocessed.view(-1, 4, states_preprocessed.size(1))
                next_states_preprocessed = next_states_preprocessed.view(-1, 4, next_states_preprocessed.size(1))
...
writer.close()
# torch.save(net.state_dict(), f"/itet-stor/ericschr/net_scratch/BA/DQN_runs/With_Pretrained_conv_BVAE/summary/{namefolder}/" + DEFAULT_ENV_NAME + f"Buff-bestreward{best_mean_reward}-{x.day}_{x.month}.dat")
